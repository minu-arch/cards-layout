import { Box, HStack, Stack } from "styled-system/jsx"
import { Text } from "~/ui/text"

const SvgGreenImage = () => (
	<Box
		className="box-notification"
		w="full"
		h="100%"
		zIndex="1"
		top="0"
		bottom="0"
		right={{
			base: "0",
			"@md": "13%",
			"@lg": "0",
		}}
		margin="auto"
		position="absolute"
	>
		<Stack
			className="stack-image"
			gap="0"
			m="0"
			p="0"
			position="relative"
			top="0"
			bottom="0"
			left={{
				base: "0",
				"@md": "28",
				"@lg": "0",
			}}
			right="0"
			zIndex="0"
		>
			<HStack gap="0" p="0">
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
			</HStack>
			<HStack gap="0" p="0">
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
			</HStack>
			<HStack gap="0" p="0">
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
			</HStack>
			<HStack gap="0" p="0">
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
			</HStack>
			<HStack gap="0" p="0">
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
			</HStack>
			<HStack gap="0" p="0">
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
			</HStack>
			<HStack gap="0" p="0">
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
			</HStack>
			<HStack gap="0" p="0">
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
				<Box>
					{" "}
					<svg
						width="44"
						height="44"
						viewBox="0 0 44 44"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						id="svg-1500058624_249"
					>
						<path
							d="M43.3643 0.181461C43.3643 24.0301 24.0311 43.3633 0.182438 43.3633C0.182438 19.5146 19.5156 0.181461 43.3643 0.181461Z"
							fill="#719267"
						/>
					</svg>
				</Box>
			</HStack>
		</Stack>
	</Box>
)

const SvgGraphImgae = () => (
	<Box
		w="full"
		position="absolute"
		bottom={{
			base: "20",
			"@md": "5",
			"@xxl": "20",
		}}
		left="0"
		right="0"
	>
		<svg
			viewBox="0 0 522 123"
			fill="none"
			xmlns="http://www.w3.org/2000/svg"
			id="svg-889909105_1397"
			preserveAspectRatio="xMidYMid meet"
			style={{ maxWidth: "100%", height: "auto" }}
		>
			<path
				d="M0.159668 3.9082H521.725"
				stroke="#EDEDED"
				stroke-width="0.584546"
				stroke-dasharray="2.34 2.34"
			/>
			<path
				d="M0.159668 32.4404H521.725"
				stroke="#EDEDED"
				stroke-width="0.584546"
				stroke-dasharray="2.34 2.34"
			/>
			<path
				d="M0.159668 60.9727H521.725"
				stroke="#EDEDED"
				stroke-width="0.584546"
				stroke-dasharray="2.34 2.34"
			/>
			<path
				d="M0.159668 89.5049H521.725"
				stroke="#EDEDED"
				stroke-width="0.584546"
				stroke-dasharray="2.34 2.34"
			/>
			<path d="M0.159668 118.037H521.725" stroke="#EDEDED" stroke-width="0.584546" />
			<path
				d="M0.159668 61.7398C0.159668 61.7398 101.888 81.3981 139.153 78.7663C176.418 76.1346 219.473 44.813 260.037 37.0806C300.6 29.3483 339.478 33.0407 382.731 27.3931C425.984 21.7455 521.725 3.9082 521.725 3.9082"
				stroke="#719267"
			/>
			<path
				d="M139.153 78.4296C99.8851 81.5471 0.159668 61.7063 0.159668 61.7063V118.038H521.725V3.9082C521.725 3.9082 425.183 21.7592 382.731 27.3795C340.279 32.9999 298.197 29.6605 260.037 37.0614C221.877 44.4624 178.421 75.3122 139.153 78.4296Z"
				fill="url(#svg-889909105_1397_paint0_linear_92_1136)"
			/>
			<defs>
				<linearGradient
					id="svg-889909105_1397_paint0_linear_92_1136"
					x1="244.643"
					y1="35.5945"
					x2="247.31"
					y2="108.783"
					gradientUnits="userSpaceOnUse"
				>
					<stop stop-color="#719267" stop-opacity="0.1" />
					<stop offset="1" stop-color="#719267" stop-opacity="0" />
				</linearGradient>
			</defs>
		</svg>
	</Box>
)

const Notification = () => {
	return (
		<HStack
			w="full"
			maxW={{
				base: "sm",
				"@sm": "xl",
				"@md": "3xl",
				"@lg": "4xl",
				"@xl": "5xl",
				"@xxl": "6xl",
			}}
			alignItems="center"
			justifyContent="space-between"
			gap="5"
			flexDirection={{
				base: "column",
				"@xxl": "row",
			}}
		>
			<Box
				h="350px"
				w="full"
				maxW={{
					base: "md",
					"@sm": "xl",
					"@md": "3xl",
					"@lg": "4xl",
					"@xl": "5xl",
					"@xxl": "6xl",
				}}
				position="relative"
				style={{ backgroundColor: "#fff" }}
				borderRadius="lg"
				overflow="hidden"
			>
				<SvgGreenImage />
				<Box
					bg="sage.12"
					p="2"
					borderRadius="sm"
					w={{
						base: "5/6",
						"@md": "4/6",
					}}
					h="4/5"
					position="absolute"
					top="0"
					bottom="0"
					left="0"
					right="0"
					margin="auto"
					zIndex="1"
				>
					<Stack p="1" h="100%" w="full">
						<Stack gap="-1">
							<Box>
								<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
									Brokerage Account
								</Text>
							</Box>
							<HStack alignItems="center" gap="1" justifyContent="flex-start">
								<Box>
									<Text
										as="h2"
										color="neutral.2"
										fontWeight="medium"
										fontSize={{
											base: "xs",
											"@md": "md",
										}}
									>
										$12,350.57
									</Text>
								</Box>
								<HStack gap="1">
									<Box>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											height="10px"
											width="10px"
											viewBox="0 -960 960 960"
											fill="#22c55e"
										>
											<path d="m216-160-56-56 464-464H360v-80h400v400h-80v-264L216-160Z" />
										</svg>
									</Box>
									<Text
										color="green.6"
										fontSize={{
											base: "2xs",
											"@md": "xs",
										}}
										fontWeight="medium"
										mt="-1"
									>
										$201.28 (+12.3%)
									</Text>
								</HStack>
							</HStack>
						</Stack>
						<HStack w="full" h="100%" justifyContent="space-between" gap="0">
							<Stack gap="5">
								<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
									1000
								</Text>
								<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
									750
								</Text>
								<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
									500
								</Text>
								<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
									250
								</Text>
								<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
									0
								</Text>
							</Stack>
							<Box
								display="flex"
								flexDirection="column"
								justifyContent={{
									base: "flex-end",
								}}
								w="full"
								h="100%"
								position="relative"
							>
								<SvgGraphImgae />

								<HStack justifyContent="space-between" gap="0">
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Jan
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Feb
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Mar
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Apr
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										May
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Jun
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Jul
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Aug
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Sep
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Oct
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Nov
									</Text>
									<Text as="p" fontSize="2xs" color="neutral.9" fontWeight="medium">
										Dec
									</Text>
								</HStack>
							</Box>
						</HStack>
					</Stack>
				</Box>
			</Box>
			<Box w="full">
				<Text as="h2" color="grass.7" fontWeight="medium" fontSize="lg">
					Real-Time Alerts & Notifications
				</Text>
				<Text as="p" color="grass.7" wordBreak="break-word" whiteSpace="pre-wrap">
					Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer a facilisis
					nulla. Fusce tincidunt ornare purus eget hendrerit. Integer viverra nisl
					viverra mi placerat, a vestibulum justo consectetur.
				</Text>
			</Box>
		</HStack>
	)
}

export default Notification
